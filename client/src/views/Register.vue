    ...
    <section class="form_container">
      ...
      >
        ...
        <el-form-item label="地址" prop="addr">
          <el-input v-model="registerUser.addr" placeholder="请输入地址"></el-input>
        </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios.post("/api/customer", this.registerUser).then(res => {
            // 注册成功
            ...
            if (res.data.code > 300) {
              ...
              // 将ID存入localStorage中
              const key ='id';
              localStorage.setItem(key,JSON.stringify(res.data.data.id)); 
